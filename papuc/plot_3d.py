...
def color_triangle_mesh(
    ...
    def eval_to_lab(theta, mag):
            a = my_map.a_func_of_theta(theta)
            b = my_map.b_func_of_theta(theta)
            lab_color = mag*np.array([my_map.L_max,a,b])
            return lab_color
    ...
    def eval_vert_to_lab(vert):
        ...
        for i in range(len(vert)):
            ...
            a = my_map.a_func_of_theta(theta)
            b = my_map.b_func_of_theta(theta)
            out[i] = mag*np.array([my_map.L_max,a,b])
        return out
...
def UCS_cone_3D(ax,
    ...
    for vertices, color in zip(vert_list, color_list):
        sRGB1_color = cspace_convert(color, my_map.color_space, "sRGB1")
...
#            ax.add_patch(  Polygon(vertices, color =np.clip(sRGB1_color,0,1), **triangle_style))
    ...
    theta_test = np.linspace(0,2*np.pi,my_map.min_theta_pts)

    a_test = my_map.a_func_of_theta(theta_test)
    b_test = my_map.b_func_of_theta(theta_test)

    ax.set_xlim(a_test.min(), a_test.max())
    ax.set_ylim(b_test.min(), b_test.max())
    ax.set_zlim(0, my_map.L_max)
