...
def plot_knots_on_isoluminant_slice(ax, my_map, ab_grid=512):
	...
	if color_grid:
		...
		if use_image:
			...
			lab_map = ones((na_grid, nb_grid, 3))*my_map.L_max
...
def plot_arc_length_vs_spline_parameter_t(ax, my_map):
	...
	ax.plot(t_at_theta_max_space, theta_max_space* my_map.arc_length, marker = '+', linestyle = '', color = 'k')
...
def plot_perceptual_derivative(ax, my_map, use_delta_instead = False):
	...
	ax.set_xlabel(r'Normalized Angle, $\theta$,')
	ax.set_xlim((0,1))
...
def plot_colorwheel(ax, my_map, num_theta_levels = 32, num_z_levels = 16, center = (0,0), radius=1.0, with_side_bar = True, with_deuteranomaly = False):
	...
	sRGB1_sequence = my_map(theta_levels, ones(theta_levels.shape))
	deut_sequence = clip( cspace_convert(sRGB1_sequence, cvd_space, "sRGB1"), 0, 1) # keep it in bounds
	...
	deut_radial_image = clip( cspace_convert(sRGB1_radial_image, cvd_space, "sRGB1"), 0, 1) # keep it in bounds
	...
	for ti in range(num_theta_levels):
		...
		if with_side_bar:
			...
			ax.add_patch(mpatches.Rectangle(
						xy = (xc +1.1*radius ,  yc + radius*(2.0*theta_levels[ti] - dtheta - 1.0 ) ), # bottom is dtheta/2 but we mulitply by 2 radii
						width = 0.2,
						height = radius*dtheta*2,
						color = color))
		...
		for zi in range(num_z_levels):
			...
			theta_angle = 360* theta_levels[ti]
...
def plot_small_wave_angle_test(ax, my_map, nwaves = 30, amplitude = 0.1, pixels_per_wave = 8):
	...
	ax.imshow(image,origin = 'lower', extent =      (0,  1, 1,  2), interpolation ='bilinear' )
	ax.imshow(deut_image,origin = 'lower', extent = (0, 1, 0,   1), interpolation ='bilinear' )
...
def plot_small_wave_magnitude_test(ax, my_map, nwaves = 30, amplitude = 0.1, pixels_per_wave = 8):
	...
	ax.imshow(image,origin = 'lower',      extent = (0, 1, 0.0, 1.0) , interpolation ='bilinear' )
	ax.imshow(deut_image,origin = 'lower', extent = (1, 2, 0.0, 1.0), interpolation ='bilinear' )

	ax.vlines(my_map.theta_knots, [1-line_len/2.0], [1], color = 'k', linewidth = 0.75)
	ax.vlines(my_map.theta_knots+1, [1-line_len/2.0], [1], color = 'k', linewidth = 0.75)
