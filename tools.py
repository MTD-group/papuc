...
from numpy import array, pi, linspace, mod,  append, arange, sqrt, diff, mean, clip, zeros, floor, cos, sin, ones
...
def periodic_spline(theta_knots, f_knots, period):
	### making some local copies ###
	theta_knots_local = array(theta_knots)
	f_knots_local     = array(f_knots)

	theta0 = theta_knots_local[0]

	### takes care of non_explicit periodicity oddness
	theta_knots_local = append( theta_knots_local, [theta0 + period] )
	f_knots_local = append (f_knots_local, [f_knots_local[0]+period ]  )
	...
	def my_func(theta):
		...
		values = splev(mod(theta-theta0 ,period) +theta0, tck)
		return values

	def my_func_prime(theta):
		#values = splev(theta, tck) # gotta have the modulo
		values = splev(mod(theta-theta0 ,period) +theta0, tck, der= 1)
		return values
...
def periodic_spline_test():
	...
	theta_knots  = array([ 45, 125, 230, 320, 340 ])
	...
	theta_smooth = arange(-45, 360+45,1)
...
class isoluminant_uniform_spline_colormap:
	def __init__(self,a_knots, b_knots, L_max,
		...
		def differtial_arc(t):
			return sqrt(a_func_prime(t)**2 + b_func_prime(t)**2)
		...
		t_knots_fine, dt = linspace(0,1.0,t_pts, retstep=True)
		...
		while abs_error > abs_cumulative_error_cutoff:
			...
			print('error', abs_error, 't_pts', t_pts)

		self.min_t_pts = t_pts

		print('Previous Arc Length', cum_int_old[-1],'Current Arc Length', cum_int[-1], 't_pts',  t_pts)

		self.arc_length = cum_int[-1]

		from scipy.interpolate import interp1d

		func_theta_of_t_raw = interp1d(x = t_knots_fine,   y = cum_int/cum_int[-1], kind = 'cubic')
		def func_theta_of_t(t):
			# this fixes the looping issue
			return func_theta_of_t_raw( t-floor(t))
		self.func_theta_of_t = func_theta_of_t

		func_t_of_theta_raw = interp1d(x = cum_int/cum_int[-1], y = t_knots_fine, kind = 'cubic')
		def func_t_of_theta(theta):
			# this fixes the looping issue
			return func_t_of_theta_raw( theta-floor(theta))
	...
	def compute_min_theta_points_for_uniformity(self, JND = None, 	max_delta_E_deviation = 1.0):
		...
		theta_knots_fine, dtheta = linspace(0,1,theta_pts, retstep=True)
		delta_a = diff(self.a_func_of_theta(theta_knots_fine))
		delta_b = diff(self.b_func_of_theta(theta_knots_fine))
		dE = sqrt(delta_a**2 + delta_b**2)# scalling by 1/dtheta was ugly
		dE_list = [dE]
		mean_dE =   mean(dE)
	...
	def __call__(self, theta, z, scale_alpha = False, clip_values = True, verbose = True):
		...
		z_ar = array(z)
		theta_ar =  array(theta)

		dims = len(z_ar.shape)

		if scale_alpha:
			image = zeros(list(z_ar.shape)+[4])
		else:
			image = zeros(list(z_ar.shape)+[3])
		...
		if dims == 2:
			for i in range(z_ar.shape[0]):
				for j in range(z_ar.shape[1]):
					...
					lab_color = z_ar[i,j]*array([self.L_max,a,b])
					if scale_alpha: lab_color = array([self.L_max,a,b])
					...
					if clip_values:
						if any(sRGB1_color>1) or any(sRGB1_color<0):
							sRGB1_color_clipped = clip( sRGB1_color, 0,1)
							Lab_color_clipped = cspace_convert(sRGB1_color_clipped, "sRGB1", self.color_space )
							if verbose:print ('L*a*b*/sRGB1 color', array(lab_color), '/', sRGB1_color,
								'\n-----> clipped to',  Lab_color_clipped, '/', sRGB1_color_clipped)
							sRGB1_color = sRGB1_color_clipped

					if scale_alpha: sRGB1_color = append(sRGB1_color, z_ar[i,j])
...
class isoluminant_uniform_circle_colormap:
	def __init__(self, radius = 40, L_max = 74, use_max_radius = True, color_space = default_colorspace):
		self.radius = radius
		self.L_max = L_max
	...
	def maximize_radius(self,cut_off = 0.1, verbose=False):
		...
		while (radius_max - radius_min) > cut_off:
			...
			sequence = self(theta_levels, ones(theta_levels.shape), clip_values = False)

			if np_any(sequence>1.0) or np_any(sequence<0.0):
	...
	def a_func_of_theta(self, theta):
		return  self.radius*cos(2 * pi * theta)
		#self.a_func_of_theta = a_func_of_theta

	def b_func_of_theta(self, theta):
		return  self.radius*sin(2 * pi * theta)
			...
			for i in range(z_ar.shape[0]):
					...
					if clip_values:
						if any(sRGB1_color>1) or any(sRGB1_color<0):
							sRGB1_color_clipped = clip( sRGB1_color, 0,1)
							Lab_color_clipped = cspace_convert(sRGB1_color_clipped, "sRGB1", self.color_space )
							print (self.color_space +'/sRGB1 color', array(lab_color), '/', sRGB1_color,
								'\n-----> clipped to',  Lab_color_clipped, '/', sRGB1_color_clipped)
							sRGB1_color = sRGB1_color_clipped
				...
				for j in range(z_ar.shape[1]):
					...
					if clip_values:
						if any(sRGB1_color>1) or any(sRGB1_color<0):
							sRGB1_color_clipped = clip( sRGB1_color, 0,1)
							Lab_color_clipped = cspace_convert(sRGB1_color_clipped, "sRGB1", self.color_space )
							print ('L*a*b*/sRGB1 color', array(lab_color), '/', sRGB1_color,
								'\n-----> clipped to',  Lab_color_clipped, '/', sRGB1_color_clipped)
							sRGB1_color = sRGB1_color_clipped
...
def HSV_colormap(theta, z, scale_alpha = False, clip_values = True, verbose = True):
	...
	if dims == 1:
		for i in range(z_ar.shape[0]):
			...
			sRGB1_color = array(hsv_to_rgb(*HSV_color))
